...
when isMainModule:
  import strutils, parseutils, sets, tables, sequtils, os, parseopt
  import strformat except `&`
  import winim/lean, jsony, zippy
  ...
  when defined(vcc):
    ...
    const Database = staticReadArray("winimx.db")
    let database = cast[string](Database)

  else:
    const Database = staticRead("winimx.db")
  ...
  proc initMinifier(used = false): Minifier =
    ...
    result.nodes = zippy.uncompress(database).fromJson(seq[CodeNode])
  ...
  proc getVersion(): string =
    const nimble = staticRead("winimx.nimble")
    for line in nimble.splitLines:
      var pair = line.split('=', maxsplit=1)
      if pair[0].strip(chars=Whitespace+{'#'}) == "winim_version":
...
Inputfile(s):
  This tool catches all identifiers from inputfile(s) and outputs only necessary
  definitions. The input can be nim source file or just keyword list.
  Wildcard is allowed.""" % version
